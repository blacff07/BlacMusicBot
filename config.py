...
def _fetch_bot_name(token: str, attempts: int = 3) -> str:
    """
    Fetch the bot's real first_name from Telegram's getMe endpoint.

    This runs once, synchronously, before the asyncio event loop starts (at
    Config() construction time in BlacMusic/__init__.py), so it does not
    block any concurrently running tasks. It retries a few times with a
    short backoff before giving up, since a single dropped connection at
    process startup shouldn't force a permanent fallback to the default
    branding name for the whole run.

    Falls back to '˹ʙʟᴀᴄ ᴍᴜꜱɪᴄ˼' if every attempt fails.
    """
    fallback = "˹ʙʟᴀᴄ ᴍᴜꜱɪᴄ˼"
    if not token:
        return fallback
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            data = r.json()
            if data.get("ok"):
                return data["result"].get("first_name", fallback)
            # Telegram responded but reported an error (e.g. bad token)
            last_error = RuntimeError(data.get("description", "unknown getMe error"))
            break
    ...
    if last_error is not None:
        print(f"[config] Warning: could not fetch bot name from Telegram ({last_error}); using fallback name.")
    return fallback
...
class Config:
    ...
    def __init__(self):
        ...
        self.HELP_IMG: str      = getenv("HELP_IMG",      self.START_IMG)
        self.STATS_IMG: str     = getenv("STATS_IMG",     self.PING_IMG)
