...
from BlacMusic import app, boot, config, db, lang
...
def _format_group_uptime(seconds: float) -> str:
    """Format seconds elapsed since boot as e.g. '1ᴅ:2ʜ:3ᴍ:4s' (larger units omitted if zero)."""
    seconds = int(seconds)
    parts = []
    if seconds >= 86400:
        parts.append(f"{seconds // 86400}ᴅ")
    if seconds >= 3600:
        parts.append(f"{(seconds % 86400) // 3600}ʜ")
    parts.append(f"{(seconds % 3600) // 60}ᴍ:{seconds % 60}s")
...
async def start(_, message: types.Message):
    ...
    if private:
        ...
        # first_name is untrusted user input - escape it since it's embedded
        # in an HTML-parsed message.
        _user_link = (
            "<a href='tg://user?id=" + str(message.from_user.id) + "'>"
            + escape(message.from_user.first_name or "") + "</a>"
        )
        ...
        _text = message.lang["start_gp"].format(
            app.username or "",
            app.name,
            _format_group_uptime(time.time() - boot),
        )
...
async def settings(_, message: types.Message):
    ...
    await utils.safe_text(
        message,
        message.lang["start_settings"].format(escape(message.chat.title or "")),
