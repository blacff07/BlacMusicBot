...
async def group_data_handler(client: Client, message: Message) -> None:
    ...
    try:
        ...
        # Build information text. All chat-supplied strings (title, username,
        # description, invite link) are HTML-escaped since they're untrusted
        # user input and this message is rendered with ParseMode.HTML.
        info_lines = []
        info_lines.append("<b>📊 GROUP INFORMATION</b>\n")

        # Basic info
        info_lines.append(f"<b>📌 ɴᴀᴍᴇ:</b> {escape(chat_info.title or '')}")
        info_lines.append(f"<b>🆔 ɪᴅ:</b> <code>{chat_id}</code>")

        if chat_info.username:
            info_lines.append(f"<b>🔗 ᴜꜱᴇʀɴᴀᴍᴇ:</b> @{escape(chat_info.username)}")
        ...
        if chat_info.description:
            desc = escape(chat_info.description)
            if len(desc) > 100:
                desc = desc[:100] + "..."
            info_lines.append(f"\n<b>📝 ᴅᴇꜱᴄʀɪᴘᴛɪᴏɴ:</b>\n{desc}")
        ...
        if chat_info.linked_chat:
            info_lines.append(f"\n<b>🔗 ʟɪɴᴋᴇᴅ ᴄʜᴀɴɴᴇʟ:</b> {escape(chat_info.linked_chat.title or '')}")
        ...
        if hasattr(chat_info, 'invite_link') and chat_info.invite_link:
            info_lines.append(f"\n<b>🔗 ɪɴᴠɪᴛᴇ ʟɪɴᴋ:</b> {escape(chat_info.invite_link)}")
