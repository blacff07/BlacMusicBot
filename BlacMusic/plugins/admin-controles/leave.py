...
from BlacMusic import app, db, lang, logger, userbot, config


@app.on_message(filters.command(["leave"]) & app.sudo_filter)
@lang.language()
async def _leave(_, m: types.Message) -> None:
    """
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Assistant failed to leave chat {chat_id}: {e}")
    except Exception as e:
        # If getting client fails, just continue with bot leaving
        logger.debug(f"Could not resolve assistant client for chat {chat_id}: {e}")
    ...
    try:
        ...
        await sent.edit_text(
            ...
            f"<blockquote>Failed to leave chat: {e}</blockquote>"
        )
