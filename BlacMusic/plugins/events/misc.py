...
from BlacMusic import tune, app, config, db, lang, logger, queue, tasks, userbot, yt
...
async def update_timer(length=10):
    ...
    async def _preload_next(chat_id, next_media):
        ...
        try:
            ...
            logger.debug(f"Preload error for chat {chat_id}: {e}")
        ...
        while True:
            try:
                ...
                if not any(err in error_str for err in [
                    ...
                    logger.debug(f"update_timer error for chat {chat_id}: {e}")
    ...
    while True:
        ...
        for chat_id in list(db.active_calls):
            try:
                        ...
                        if alone_duration >= LEAVE_TIMEOUT:
                            ...
                            try:
                                ...
                                if not any(ignore in error_msg for ignore in [
                                    ...
                                    logger.debug(f"Error leaving call for {chat_id}: {e}")
                            alone_times.pop(chat_id, None)
                else:
                ...
                logger.debug(f"vc_watcher error for chat {chat_id}: {e}")
                alone_times.pop(chat_id, None)
                continue
