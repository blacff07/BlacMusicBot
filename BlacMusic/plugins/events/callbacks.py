...
async def handle_seek(query: types.CallbackQuery, chat_id: int, action: str, user: str):
    ...
    if new_time == 0 and seconds < 0:
        return await query.answer("⏮️ ᴀʟʀᴇᴀᴅʏ ᴀᴛ ᴛʜᴇ ʙᴇɢɪɴɴɪɴɢ!", show_alert=True)
    if new_time >= media.duration_sec - 5 and seconds > 0:
        return await query.answer("⏭️ ᴛᴏᴏ ᴄʟᴏꜱᴇ ᴛᴏ ᴛʜᴇ ᴇɴᴅ!", show_alert=True)
    ...
    if success:
        ...
        try:
            _seek_reply = (
                "<blockquote>⏩ ꜱᴇᴇᴋᴇᴅ " + label + " → <b>" + time_str + "</b>" +
