...
from BlacMusic import app, config, db


@app.on_message(filters.command(["channelplay"]) & filters.group & ~app.bl_users)
async def channelplay_command(_, m: Message) -> None:
    ...
    if len(m.command) < 2:
        return await m.reply_text(
            f"ᴄʜᴀɴɴᴇʟ ᴘʟᴀʏ ꜱᴇᴛᴛɪɴɢꜱ ꜰᴏʀ {escape(m.chat.title or '')}\n\n"
    ...
    # Enable for linked channel
        ...
        if chat.linked_chat:
            ...
            return await m.reply_text(
                f"✅ ᴄʜᴀɴɴᴇʟ ᴘʟᴀʏ ᴇɴᴀʙʟᴇᴅ ꜰᴏʀ: {escape(chat.linked_chat.title or '')}\n"
    ...
    # Enable for specific channel
        ...
        if owner_id != m.from_user.id:
            return await m.reply_text(
                f"❌ ʏᴏᴜ ɴᴇᴇᴅ ᴛᴏ ʙᴇ ᴛʜᴇ ᴏᴡɴᴇʀ ᴏꜰ ᴄʜᴀɴɴᴇʟ {escape(chat.title or '')} ᴛᴏ ᴄᴏɴɴᴇᴄᴛ ɪᴛ ᴡɪᴛʜ ᴛʜɪꜱ ɢʀᴏᴜᴘ.\n\n"
                f"ᴄʜᴀɴɴᴇʟ'ꜱ ᴏᴡɴᴇʀ: @{escape(owner_username or '')}\n\n"
                "ᴀʟᴛᴇʀɴᴀᴛɪᴠᴇʟʏ, ʏᴏᴜ ᴄᴀɴ ʟɪɴᴋ ʏᴏᴜʀ ᴄʜᴀᴛ'ꜱ ᴄʜᴀɴɴᴇʟ ᴀɴᴅ ᴄᴏɴɴᴇᴄᴛ ᴡɪᴛʜ `/channelplay linked`"
            )

        await db.set_cmode(m.chat.id, chat.id)
        return await m.reply_text(
            f"✅ ᴄʜᴀɴɴᴇʟ ᴘʟᴀʏ ᴇɴᴀʙʟᴇᴅ ꜰᴏʀ: {escape(chat.title or '')}\n"
