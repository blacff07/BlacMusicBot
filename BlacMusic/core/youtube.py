...
class YouTube:
    ...
    def get_cookies(self):
        if not self.checked:
            try:
                ...
                logger.warning("Cookies directory 'BlacMusic/cookies' does not exist; creating it.")
                os.makedirs("BlacMusic/cookies", exist_ok=True)
            self.checked = True
        if not self.cookies:
            if not self.warned:
                self.warned = True
    ...
    async def search(self, query: str, m_id: int) -> Track | None:
        ...
        if results and results["result"]:
            ...
            thumbnail_url = (thumbnails[-1].get("url") or "").split("?")[0]
        ...
        try:
            ...
            for data in plist["videos"][:limit]:
                try:
                    ...
                    # Skip this entry but keep processing the rest of the playlist
                    logger.debug(f"Skipping unparsable playlist entry: {e}")
                    continue
            ...
            raise Exception(
                "Failed to parse playlist. YouTube may have changed their structure."
            ) from e
        except Exception:
