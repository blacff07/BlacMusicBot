...
from BlacMusic.helpers import Media, buttons, utils
...
class Telegram:
    ...
    async def download(self, msg: types.Message, sent: types.Message) -> Media | None:
        ...
        # File extension: fall back to a sensible default based on media type
        # when Telegram doesn't supply a file_name (voice notes in particular
        # have no file_name, which previously produced a broken "file." path
        # with no extension at all).
        _raw_name = getattr(media, "file_name", "") or ""
        if "." in _raw_name:
            file_ext = _raw_name.rsplit(".", 1)[-1]
        elif msg.voice:
            file_ext = "ogg"
        elif is_video:
            file_ext = "mp4"
        ...
        try:
            ...
            # Remove the partially-downloaded file so cancelled downloads
            # don't leave orphaned data behind in downloads/.
            try:
