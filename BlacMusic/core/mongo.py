...
from BlacMusic import config, logger, userbot
...
class MongoDB:
    def __init__(self):
        ...
        # These three defaults exist so direct attribute reads never crash
        # before the first DB round-trip, but the *_loaded flags below are
        # what actually gate the lazy DB load in their getters. Without a
        # separate flag, `hasattr(self, "maintenance")` (etc.) would always
        # be True from the moment __init__ runs, and the getters would never
        # reload the real persisted value from MongoDB after a restart.
        self.maintenance = False  # Maintenance mode status
        self._maintenance_loaded = False
        self.gbanned_users = []  # Globally banned users
        self._gbanned_loaded = False
        self.vplay_enabled = config.VIDEO_PLAY
        self._vplay_loaded = False
        ...
        # Autoplay in-memory caches (no DB collection needed for last_query)
    ...
    async def get_admins(self, chat_id: int, reload: bool = False) -> list[int]:
        from BlacMusic.helpers._admins import reload_admins
    ...
    async def get_maintenance(self) -> bool:
        ...
        if not self._maintenance_loaded:
    ...
    async def get_autoplay(self, chat_id: int) -> bool:
        """Return autoplay enabled status for a chat."""
        if chat_id in self._autoplay_cache:
            return self._autoplay_cache[chat_id]
        doc = await self.cache.find_one({"_id": f"autoplay_{chat_id}"})
        val = bool(doc.get("enabled", False)) if doc else False
        self._autoplay_cache[chat_id] = val
        return val

    async def set_autoplay(self, chat_id: int, enabled: bool) -> None:
        """Enable or disable autoplay for a chat."""
        self._autoplay_cache[chat_id] = enabled
        if enabled:
            await self.cache.update_one(
    ...
    async def get_last_query(self, chat_id: int) -> str | None:
        """Return the last user search query (used to seed autoplay)."""
        return self._last_query.get(chat_id)

    async def set_last_query(self, chat_id: int, query: str) -> None:
        """Store the last user search query for autoplay seeding."""
