...
class Thumbnail:
    ...
    async def generate(self, song: Track, size=(1280, 720)) -> str:
        try:
            temp   = f"cache/temp_{song.id}_{uuid.uuid4().hex}.jpg"
            ...
            logger.debug(f"Thumbnail generation failed for {song.id}: {e}")
    ...
    def _generate_sync(self, temp, output, song, size=(1280, 720)) -> str:
        try:
            ...
            logger.debug(f"Thumbnail sync generation failed for {song.id}: {e}")
            return config.DEFAULT_THUMB